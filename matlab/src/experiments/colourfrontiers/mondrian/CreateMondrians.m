...
%be the final value.
Mondrian1 = zeros(500, 500);
Mondrian2 = zeros(500, 500);
Mondrian3 = zeros(500, 500);

NumberOfLayers= 4; %How many times we are going to fill and refill each surface of the mondrian.
...
for h=1:NumberOfMondrians
  ...
  for i=1:NumberOfLayers
    ...
    for j=1:AllPositionsInMatrix
      ...
      RandomColor1 = randperm (length(Colors));
      RandomColor2 = randperm (length(Colors));
      RandomColor3 = randperm (length(Colors));
      ...
      Mondrian1( VertInitial : VertInitial+ Dimensions(RandomHorizontalSize(1)) , ...
        HorizInitial : HorizInitial + Dimensions(RandomVerticalSize(1)) ) = Colors(RandomColor1(1));
      Mondrian2( VertInitial : VertInitial+ Dimensions(RandomHorizontalSize(1)) , ...
        HorizInitial : HorizInitial + Dimensions(RandomVerticalSize(1)) ) = Colors(RandomColor2(1));
      Mondrian3( VertInitial : VertInitial+ Dimensions(RandomHorizontalSize(1)) , ...
        HorizInitial : HorizInitial + Dimensions(RandomVerticalSize(1)) ) = Colors(RandomColor3(1));
    end
    %As the matrix is going to be expanded to the right we need to
    %do a final crop to the image.
    Mondrian1 = uint8(Mondrian1(1:FinalSizeHeight, 1:FinalSizeWidth));
    Mondrian2 = uint8(Mondrian2(1:FinalSizeHeight, 1:FinalSizeWidth));
    Mondrian3 = uint8(Mondrian3(1:FinalSizeHeight, 1:FinalSizeWidth));
  end
  %Fill the Array of Mondrians with each Mondrian.
  ArrayOfMondrians{h, 1} =  Mondrian1; %#ok
  ArrayOfMondrians{h, 2} =  Mondrian2; %#ok
  ArrayOfMondrians{h, 3} =  Mondrian3; %#ok
end
