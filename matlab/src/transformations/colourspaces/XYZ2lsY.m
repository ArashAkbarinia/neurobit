...
switch choice
  ...
  case 'hpe'
    ...
    T = [0.4002 0.7076 -0.0808; -0.2263 1.1653 0.0457; 0 0 0.9182];
  case 'macleod-boynton1986'
    ...
    T = [0.15516 0.54308 -0.03287; -0.15516 0.45692 0.03287; 0 0 1.0066];
  case 'vos-valraven'
    %Includes the condition that Y= L+M+S
    %XYZ is derived from the Judd-modified CIE 2-deg color matching
    %functions (1951)
    T = [0.15516 0.54308 -0.03702; -0.15516 0.45692 0.02969; 0 0 0.00732];
  case 'smith-pokorny'
    ...
    T = [0.15514 0.54312 -0.03286; -0.15514 0.45684 0.03286; 0 0 0.00801];
  case 'macleod-boynton1979'
    %They assume the condition that
    %L(400)+M(400)=S(400)
    % and XYZ is derived from the Judd-modified CIE 2-deg color matching
    %functions (1951)
    T = [0.15514 0.54312 -0.03286; -0.15514 0.45684 0.03286; 0 0 0.01608];
  case 'evenly_ditributed_stds'
    ...
    %patches.
    T = [0.15516 0.54307 -0.03287; -0.15516 0.45692 0.03287; 0 0 0.05930];
  otherwise
...
% lsY = zeros(n,m);
% for i=1:n
%     lsY(i,1) = LMS(i,1)./sum(LMS(i,1:2)); % l = L/(L+M)
%     lsY(i,2) = LMS(i,3)./sum(LMS(i,1:2)); % s = S/(L+M)
%     lsY(i,3) = XYZ(i,2);                % Y = Y;
% end
