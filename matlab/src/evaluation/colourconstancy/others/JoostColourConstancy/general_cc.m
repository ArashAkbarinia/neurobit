...
if mink_norm > -1          % minkowski norm = (1,infinity >
  ...
  white_G=white_G/som;
  white_B=white_B/som;
elseif mink_norm == -1      %minkowski-norm is infinit: Max-algorithm
  R=input_data(:,:,1);
  G=input_data(:,:,2);
  B=input_data(:,:,3);
  ...
  white_G=white_G/som;
  white_B=white_B/som;
elseif mink_norm == -2      %contrast variant max pooling
  luminance = ContrastVariantPooling(input_data, InputImage);
  white_R = luminance(1);
  white_G = luminance(2);
  white_B = luminance(3);
end
