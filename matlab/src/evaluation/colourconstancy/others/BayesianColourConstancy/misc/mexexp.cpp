...
void mexexp(const double*y, double*yp, int m) 
{
...
{ 
    ...
    if (nrhs != 1) 
        mexErrMsgTxt("mexexp: only one input argument required."); 
    else if (nlhs > 1)
        mexErrMsgTxt("mexexp: too many output arguments."); 
			...
			for (unsigned int indc=0;indc<n;indc++)
			{
				...
				while (indx<next_indx)
				{
					...
					for (unsigned int cnt=0;cnt<m;cnt++)
					{
						if (cnt==next_ir)
						{
							mexexp(yi+indx,ypi+mindc+cnt,1);
							indx++;
							next_ir = ir_in[indx];
						}
						else /* zero at that point: exp(0) = 1 */
